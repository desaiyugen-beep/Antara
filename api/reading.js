...
export default async function handler(req, res) {
  ...
  try {
    ...
    // some fields can be a string OR an object like {name:"..."}; this reads either
    const pick = (...vals) => {
      for (const v of vals) {
        if (v == null) continue;
    ...
    // map KundliAPI fields -> the shape the page expects (covers naming variants)
    const reading = {
      name: name || 'friend',
      vedicSun: pick(a.sunSign, a.sun_sign, a.sunsign),                 // sidereal Sun
      moon:     pick(a.moonSign, a.sign, a.moon_sign, a.rashi),          // Moon / Rashi
      nak:      pick(a.nakshatra, a.nakshatraName, a.nakshatra_name,
                     a.star, a.birthStar, a.naksatra),                   // birth star
      ascendant: pick(a.ascendant, a.lagna, a.asc),                      // Lagna
